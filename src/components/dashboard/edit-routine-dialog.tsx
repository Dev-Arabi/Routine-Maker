...
import type { DayOfWeek, Routine } from "@/lib/types"
import { Plus, Trash2, ArrowRight, CalendarDays, Clock, BookOpen, Users, CheckCircle, Pencil } from "lucide-react"
...
interface TimeSlot {
  id?: string // Optional for new slots, required for existing
  day: DayOfWeek
  subject: string
  startTime: string
  endTime: string
...
export function EditRoutineDialog({ open, onOpenChange, initialRoutine, onRoutineUpdated }: EditRoutineDialogProps) {
  ...
  useEffect(() => {
    if (initialRoutine) {
      ...
      setSelectedClass(initialRoutine.class_number)
      // Map existing routine entries to TimeSlot format
      const mappedTimeSlots: TimeSlot[] =
        initialRoutine.routine_entries?.map((entry) => ({
          id: entry.id,
          day: entry.day_of_week as DayOfWeek,
      ...
      // Reset if no initial routine (e.g., dialog closed and reopened)
      resetForm()
    }
  }, [initialRoutine])
  ...
  const handleSubmit = async () => {
    if (!routineName || !selectedClass || !initialRoutine) return // initialRoutine must exist for update
    ...
    try {
      ...
      // Delete existing routine entries for this routine
      ...
      // Insert new/updated routine entries
      const entriesToInsert = timeSlots.map((slot) => ({
      ...
      const { error: insertEntriesError } = await supabase.from("routine_entries").insert(entriesToInsert)

      if (insertEntriesError) throw insertEntriesError

      onRoutineUpdated() // Notify parent to refetch routines
      handleClose()
    } catch (error) {
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white/95 backdrop-blur-md border-indigo-200 shadow-2xl">
        <DialogHeader className="bg-gradient-to-r from-indigo-50 to-indigo-100 -m-6 mb-6 p-6 rounded-t-lg border-b border-indigo-200">
          <DialogTitle className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-indigo-800 flex items-center">
            <Pencil className="h-6 w-6 mr-3 text-indigo-600" />
            Edit Routine: {initialRoutine?.name}
          </DialogTitle>
          <DialogDescription className="text-indigo-600 font-medium">
            Step {step} of 3: {step === 1 ? "Edit Class" : step === 2 ? "Edit Time Slots" : "Confirm Changes"}
          </DialogDescription>
        </DialogHeader>
            ...
            <Card className="backdrop-blur-sm bg-gradient-to-br from-indigo-50/80 to-white/80 border-indigo-200 shadow-lg">
              <CardHeader className="bg-gradient-to-r from-indigo-100/50 to-indigo-50/50">
                ...
                <DialogDescription className="text-indigo-600">
                  Modify the class number for this routine.
                </DialogDescription>
              </CardHeader>
          ...
          <div className="space-y-6">
            <Card className="backdrop-blur-sm bg-gradient-to-br from-indigo-50/80 to-white/80 border-indigo-200 shadow-lg">
              <CardHeader className="bg-gradient-to-r from-indigo-100/50 to-indigo-50/50">
                ...
                <DialogDescription className="text-indigo-600">Add, remove, or modify class periods.</DialogDescription>
              </CardHeader>
            ...
            <Card className="backdrop-blur-sm bg-gradient-to-br from-indigo-50/80 to-white/80 border-indigo-200 shadow-lg">
              <CardHeader className="bg-gradient-to-r from-indigo-100/50 to-indigo-50/50">
                <CardTitle className="text-indigo-800 flex items-center">
                  ...
                  Confirm Routine Details
                </CardTitle>
                <DialogDescription className="text-indigo-600">Review and confirm your changes.</DialogDescription>
              </CardHeader>
              <CardContent className="pt-6">
                <div>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-indigo-600 bg-white/40 p-3 rounded-lg">
                    Review the details before updating your routine.
                  </p>
                </div>
              </CardContent>
            </Card>
